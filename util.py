...
class brainvision(BaseConcatDataset):
    """
    Parameters
    ----------
    path: str
    ...
    def __init__(self, path, recording_ids=None, target_name=None,
        ...
        file_paths = glob.glob(os.path.join(path, '**/*.vhdr'), recursive=True)
    ...
    def _create_dataset(description, target_name, preload,
        ...
        raw=channel_processing(mne.io.read_raw_brainvision(file_path, preload=preload, verbose=False))
        cz = mne.pick_channels(raw.info["ch_names"], ['EEG FC1-REF', 'EEG FC2-REF', 'EEG CP1-REF', 'EEG CP2-REF'])
        # print(cz)
        ...
        channels = [
            ...
            'EEG T6-REF', 'EEG FZ-REF', 'EEG PZ-REF', ]

        channel_dict = {channel: mne.pick_channels(raw.info["ch_names"], [channel]) for channel in channels}
        channel_dict['EEG CZ-REF'] = cz

        # print('ori_channel',raw.info['ch_names'])
        raw = mne.channels.combine_channels(raw, channel_dict, method="mean")
        # print('after_channel',raw.info['ch_names'])
...
def _parse_description_from_file_path(file_path):
    ...
    sleep_restricted = 0 if file_path[-6] == 'b' else 1
    subject =tokens[-1][:-6]
    task = tokens[-2]
    # version = tokens[-7]
    ...
    return {
        ...
        'subject': subject,
        'task': task,
        'sleep_restricted': int(sleep_restricted),
    }
...
def load_brainvision_as_windows(data_folder):
    ...
    print('paths',paths)
    targets=[0 if path[-6] == 'b' else 1 for path in paths]
    ...
    recording_dataset = BaseDataset()
    des=windows_dataset.description
    des['path']=paths
    # print('des',des)
    windows_dataset.set_description(des, overwrite=True)
...
def select_by_duration(ds, tmin=0, tmax=None):
    ...
    for d_i, d in enumerate(ds.datasets):
        ...
        if tmin <= duration <= tmax:
            ...
            print('Recording {} has length {}s and is excluded'.format(d_i, duration))
    splits = ds.split(split_ids)
    split = splits['0']
    return split
...
def split_data(windows_ds, split_way, train_size, valid_size, test_size, shuffle, random_state,remove_attribute=None,train_percent=1):
    if split_way == 'proportion':
        ...
        idx_valid, idx_test = train_test_split(idx_valid_test, random_state=random_state, test_size=test_size/(test_size+valid_size),
                                               shuffle=shuffle)
        idx_train = idx_train[:int(len(idx_train) * train_percent)]
