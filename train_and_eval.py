...
from septr import SeparableTr1d
...
with open(log_path,'a') as f:
    ...
    writer.writerow(['train_loss', 'valid_loss',  'train_accuracy',  'valid_accuracy','etl_time','model_training_time',\
     ...
     'acc_per_recording','mcc','mcc_per_recording','activation','remove_attribute','deep4_stride_before_pool','pretrain','test_stride'])
...
for (random_state,tuab,tueg,n_tuab,n_tueg,n_load,preload,window_len_s,\
     ...
     relabel_dataset,relabel_label,channels,remove_attribute,activation,test_stride,train_percent,load_brainvision,n_brainvision) in product(
            ...
            TRAIN_SIZE,VALID_SIZE,TEST_SIZE,SHUFFLE,WINDOW_STRIDE_SAMPLES,RELABEL_DATASET,RELABEL_LABEL,CHANNELS,REMOVE_ATTRIBUTE,ACTIVATION,TEST_STRIDE,TRAIN_PERCENT,LOAD_BRAINVISION,N_BRAINVISION):
    ...
    channels,remove_attribute,activation,test_stride,train_percent)
    ...
    if load_saved_windows:
        ...
        if load_saved_data:
            ...
            if not load_brainvision:
                ...
                brainvision_ids = list(range(n_tuab)) if n_brainvision else None
                ds = brainvision(path=brainvision_path, preload=preload, target_name='sleep_restricted', n_jobs=n_jobs,recording_ids=brainvision_ids)
                print(ds.datasets[10].raw.info['ch_names'])
        ...
        for sub_ds in windows_ds.datasets:
            ...
            assert sub_ds.windows.preload == preload
    ...
    for (i, n_classes, lr, weight_decay, batch_size, n_epochs, model_name, final_conv_length,dropout) \
      ...
      FINAL_CONV_LENGTH,DROPOUT):
        ...
        if shuffle:
            ...
            # set train_set as global variable

            print('len_train', len(train_set.description.loc[:, ['path']]))

            train_set, valid_set, test_set = split_data(windows_ds, split_way, train_size, valid_size, test_size,
                                                        shuffle, random_state+i,remove_attribute,train_percent)
            print('len_train', len(train_set.description.loc[:, ['path']]) )
        ...
        def exp(dropout=0.2):
            ...
            if model_name=='deep4':
                ...
                model = ViT(num_channels=n_channels,input_window_samples = window_len_samples,patch_size = vit_patch_size,num_classes = n_classes,dim = vit_dim,depth = vit_depth,heads = vit_heads,mlp_dim = vit_mlp_dim,dropout = dropout,emb_dropout = vit_emb_dropout,dim_head=64)
                ...
                # (hparams.cfg)['audio_config']['sample_rate'] = 1600
                print(hparams.cfg)
                ...
                if other_pretrain:
                    ...
                    # with open(log_path, 'a') as f:
                    #     writer = csv.writer(f, delimiter=',', lineterminator='\n', )
                    #     writer.writerow(['other_pretrain'])
            ...
            elif model_name=='septr':
                model=SeparableTr1d(channels=21, input_size=(224, 224), num_classes=2, depth=3, heads=5, mlp_dim=40, dim_head=64, down_sample_input=None, dim=10,device=device)
            ...
            if not test_model and train_percent:
            ...
            if test_on_brainvision:
                windows_brainvision=load_brainvision_as_windows(brainvision_path)
            ...
            if test_stride:
                print('ds', ds, type(ds))
                windows_ds_overlap = create_fixed_length_windows(
                    ds, start_offset_samples=0, stop_offset_samples=None,
                    window_size_samples=window_len_samples,
                    window_stride_samples=test_stride, drop_last_window=True,
                    preload=preload, drop_bad_windows=True, n_jobs=n_jobs)

                for sub_ds in windows_ds_overlap.datasets:
                    sub_ds.windows.drop_bad()
                    assert sub_ds.windows.preload == preload

                train_set, valid_set, test_set = split_data(windows_ds_overlap, split_way, train_size, valid_size, test_size,
            ...
            with open(log_path, 'a') as f:
                ...
                if not test_model and train_percent:
                 ...
                 sampling_freq,test_on_eval,split_way,train_size,valid_size,test_size,shuffle,\
                 model_name,final_conv_length,window_stride_samples,relabel_dataset,relabel_label,\
                 channels,dropout, precision_per_recording,recall_per_recording,acc_per_recording,mcc,mcc_per_recording,activation,remove_attribute,deep4_stride_before_pool,pretrain,test_stride,train_percent,other_pretrain])
                else:
                 ...
                 sampling_freq,test_on_eval,split_way,train_size,valid_size,test_size,shuffle,\
                 model_name,final_conv_length,window_stride_samples,relabel_dataset,relabel_label,\
                 channels,dropout, precision_per_recording,recall_per_recording,acc_per_recording,mcc,mcc_per_recording,activation,remove_attribute,deep4_stride_before_pool,pretrain,test_stride,train_percent])
            ...
            if train_whole_dataset_again:
                with open('./training_detail.csv', 'a') as f1:
                    ...
                    if need_feature:
                        ...
                        model.linear2.register_forward_hook(hook)
                        clf.module = model
                    ...
                    if need_feature:
                        ...
                        starts=find_all_zero(train_set.get_metadata()['i_window_in_trial'].tolist())+[x+len_train for x in find_all_zero(valid_set.get_metadata()['i_window_in_trial'].tolist())]+[y+len_valid_train for y in find_all_zero(test_set.get_metadata()['i_window_in_trial'].tolist())]

                        feature_and_label={'feature':feature,'label':windows_true,'start':starts}
                        print('feature.shape',feature.shape)
                        feature_path = './save_feature/save_feature'+time.strftime('%Y-%m-%d_%H_%M_%S',time.localtime(time.time()))+'.pth'
                        torch.save(feature_and_label, feature_path)
                        # feature_and_label_load = torch.load(feature_path)
                        # print('feature_load.shape',feature_and_label_load['feature'].shape)
                        # print('label_load.shape',len(feature_and_label_load['label']))
